...
const instance=createMovementServer(authority.world,port,authority,5000,conditions,host);
instance.server.on('listening',()=>{console.log(`INKDAYS coop: ws://${host}:${port} (2 jogadores, combate autoritativo${badNet?', rede degradada':''})`);if(lan)for(const entries of Object.values(networkInterfaces()))for(const entry of entries??[])if(entry.family==='IPv4'&&!entry.internal)console.log(`Abra nos aparelhos: http://${entry.address}:5180/?coop=1`);});
