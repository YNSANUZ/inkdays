...
export class Horde {
  ...
  update(dt:number,active:number,spawn:()=>boolean,bossHealthRatio:number|null=null){
    ...
    const remaining=this.remaining(active);this.lastEnemyElapsed=remaining>0&&remaining<=3&&this.spawned===this.goal?this.lastEnemyElapsed+dt:0;return remaining===0;
  }
  ...
  state(active:number):HordeState{const remaining=this.remaining(active);return {total:this.goal,spawned:this.spawned,killed:this.killed,remaining,active,queued:this.queued,wave:Math.max(0,this.waveIndex+1),waves:this.plan.waves.length,complete:remaining===0,assist:remaining>0&&remaining<=3&&this.lastEnemyElapsed>=C.horde.lastEnemyAssist,boss:this.plan.boss};}
}
