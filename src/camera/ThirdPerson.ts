...
export class ThirdPerson {
  camera=new T.PerspectiveCamera(C.camera.fov,16/9,.08,190); yaw=0; pitch=.19; shake=0;mode:CameraMode='third';
  ...
  update(dt:number,pos:T.Vector3,world:World,immediate=false) {
    ...
    if(this.mode==='first'){
      this.camera.position.copy(target).addScaledVector(f,.08);this.initialized=true;
      ...
      this.camera.updateMatrixWorld();return;
    }
