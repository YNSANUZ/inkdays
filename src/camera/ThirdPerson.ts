...
export class ThirdPerson {
  ...
  update(dt:number,pos:T.Vector3,world:World) {
    ...
    if(!this.initialized||hits.length) {this.camera.position.copy(desired);this.initialized=true;} else this.camera.position.lerp(desired,1-Math.exp(-C.camera.smoothing*dt));
