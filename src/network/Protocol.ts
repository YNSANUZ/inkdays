...
export interface InputPacket { version:1; sequence:number; yaw:number; pitch?:number; command:Command }
...
export function parseInput(value:unknown):InputPacket|null{
  ...
  if(Object.keys(p).some(k=>!['version','sequence','yaw','pitch','command'].includes(k)))return null;
  ...
  return {version:1,sequence:p.sequence as number,yaw:p.yaw,pitch:p.pitch as number|undefined,command:{x:c.x as number,z:c.z as number,run:c.run as boolean,crouch:c.crouch as boolean,jump:c.jump as boolean,fire:c.fire as boolean,reload:c.reload as boolean}};
}
