...
export interface InputPacket { version:1; sequence:number; yaw:number; pitch?:number; viewTick?:number; shotId?:number; command:Command }
...
export function parseInput(value:unknown):InputPacket|null{
  ...
  if(Object.keys(p).some(k=>!['version','sequence','yaw','pitch','viewTick','shotId','command'].includes(k)))return null;
  ...
  return {version:1,sequence:p.sequence as number,yaw:p.yaw,pitch:p.pitch as number|undefined,viewTick:p.viewTick as number|undefined,shotId:p.shotId as number|undefined,command:{x:c.x as number,z:c.z as number,run:c.run as boolean,crouch:c.crouch as boolean,jump:c.jump as boolean,fire:c.fire as boolean,reload:c.reload as boolean}};
}
