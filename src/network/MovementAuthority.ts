...
export class MovementAuthority {
  ...
  receive(id:string,value:unknown){
    ...
    if(!player||!packet||packet.sequence<=player.received)return false;
  ...
  snapshot(){return {version:PROTOCOL_VERSION,tick:this.tick,players:Array.from(this.players,([id,p])=>({id,acknowledged:p.applied,yaw:p.input.yaw,position:{...p.motion.position},velocity:{...p.motion.velocity},vertical:p.motion.vertical}))};}
}
