...
export class ClientPrediction {
  motion:Motion;renderPosition:Point;private pending:PredictionInput[]=[];private correction={x:0,y:0,z:0};private reliable:{jump:number|null;reload:number|null;shot:number|null}={jump:null,reload:null,shot:null};
  ...
  prepare(sequence:number,command:Command){
    if(command.jump)this.reliable.jump=sequence;if(command.reload)this.reliable.reload=sequence;if(command.shot)this.reliable.shot=sequence;const within=(start:number|null)=>start!==null&&sequence-start<=30,{shot:_,...base}=command;void _;
    return {...base,jump:within(this.reliable.jump),reload:within(this.reliable.reload)};
  }
  ...
  reconcile(state:AuthoritativeMotion){
    const before={...this.renderPosition};for(const action of ['jump','reload','shot'] as const)if(this.reliable[action]!==null&&this.reliable[action]!<=state.acknowledged)this.reliable[action]=null;this.pending=this.pending.filter(i=>i.sequence>state.acknowledged);this.motion=cloneMotion(state);
  ...
  get shotId(){return this.reliable.shot??undefined;}
