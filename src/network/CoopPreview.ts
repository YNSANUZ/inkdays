...
export function mountCoopPreview(app:HTMLElement){
  ...
  const hud=document.createElement('div');hud.className='coop-hud';hud.innerHTML='<div class="coop-boss" hidden><b>O COLOSSO</b><i><em></em></i><span></span></div><div class="coop-day"><b>☼ &nbsp; DIA 1</b><span>PREPARAÇÃO · 00:40</span></div><div class="coop-stats"><b class="coop-money">$ 0</b><span class="coop-kills">✕ 0</span><small>CHEFÃO · DIA 10</small></div><div class="coop-health"><b>VOCÊ</b><span class="coop-hearts">♥ ♥ ♥</span><i><em></em></i><small>100 / 100</small></div><div class="coop-ammo"><small>PISTOLA</small><b>8 <span>/ 48</span></b><i></i></div>';app.append(hud);
  const panel=document.createElement('div');panel.className='coop-panel';
  panel.innerHTML='<strong>COOP · TESTE DE COMBATE</strong><p role="status">Conectando…</p><label class="nickname">SEU NOME <input maxlength="16" autocomplete="nickname"></label><button>CONTINUAR</button><p class="help">WASD · mouse · R recarrega<br>Esc libera o mouse; a partida continua.<br>Após a derrota, qualquer jogador pode reiniciar a sala.</p><details><summary>REDE</summary><button class="report">BAIXAR RELATÓRIO</button></details>';app.append(panel);
  ...
  let id='',snapshot:Snapshot|null=null,snapshotReceivedAt=0,sequence=0,last=0,connected=false,prediction:ClientPrediction|null=null,telemetry=new SnapshotTelemetry(),rtt=0,maxCorrection=0,snaps=0,round=0,lastReloading:boolean|null=null;
  ...
  let socket:WebSocket,reconnectTimer=0,restartTimer=0,reviveTimer=0,pageLeaving=false,roomFull=false;
  ...
  const connect=()=>{
    ...
    if(packet.type==='snapshot'){
      ...
      const announcement=phaseEvents.observe({round:state.round,day:state.day,phase:state.phase});if(announcement){phaseBanner.innerHTML=announcement.kind==='horde'?`<b>HORDA</b><span>DIA ${announcement.day}</span>`:`<b>DIA ${announcement.day}</b><span>AMANHECEU</span>`;phaseBanner.classList.remove('visible','horde');void phaseBanner.offsetWidth;if(announcement.kind==='horde')phaseBanner.classList.add('horde');phaseBanner.classList.add('visible');audio.cue('ui');}
      ...
      const seconds=Math.max(0,Math.ceil(state.remaining)),health=Math.max(0,me?.health??0),hearts=Math.ceil(health/(C.player.health/3));hud.querySelector('.coop-day b')!.textContent=`${state.phase==='horde'?'☾':'☼'}   DIA ${state.day}`;hud.querySelector('.coop-day span')!.textContent=`${state.phase==='horde'?'HORDA':'PREPARAÇÃO'} · ${String(Math.floor(seconds/60)).padStart(2,'0')}:${String(seconds%60).padStart(2,'0')}`;hud.querySelector('.coop-money')!.textContent=`$ ${me?.money??0}`;hud.querySelector('.coop-kills')!.textContent=`✕ ${me?.kills??0}`;hud.querySelector('.coop-stats small')!.textContent=`CHEFÃO · DIA ${nextBoss(state.day)}`;hud.querySelector('.coop-hearts')!.textContent=[0,1,2].map(index=>index<hearts?'♥':'♡').join(' ');(hud.querySelector('.coop-health em') as HTMLElement).style.width=`${health}%`;hud.querySelector('.coop-health small')!.textContent=`${Math.ceil(health)} / ${C.player.health}`;hud.querySelector('.coop-ammo b')!.innerHTML=`${me?.ammo??0} <span>/ ${me?.reserve??0}</span>`;hud.querySelector('.coop-ammo i')!.textContent=me?.reloading?'RECARREGANDO…':'';hud.classList.toggle('horde',state.phase==='horde');const bossHud=hud.querySelector<HTMLElement>('.coop-boss')!;bossHud.hidden=!state.boss;bossHud.classList.toggle('enraged',!!state.boss?.enraged);if(state.boss){bossHud.querySelector('b')!.textContent=`DIA ${state.day} — ${state.boss.name}${state.boss.enraged?' · FÚRIA':''}`;(bossHud.querySelector('em') as HTMLElement).style.width=`${Math.max(0,state.boss.health/state.boss.maxHealth*100)}%`;bossHud.querySelector('span')!.textContent=`${Math.ceil(state.boss.health)} / ${state.boss.maxHealth}`;}
      revive.hidden=me?.health!==0;if(me?.health!==0&&reviveTimer){clearInterval(reviveTimer);reviveTimer=0;reviveButton.disabled=false;reviveButton.textContent='REVIVER AGORA';}
      status.textContent=`DIA ${state.day} · ${state.phase==='horde'?'HORDA':'PREPARAÇÃO'} ${Math.ceil(state.remaining)}s · ${state.players.filter(p=>p.connected).length}/${MAX_PLAYERS} | Vida ${me?.health??0} · ${me?.ammo??0}/${me?.reserve??0} ${me?.reloading?'RECARREGANDO':''} · $${me?.money??0} · ping ${rtt.toFixed(0)}ms · jitter ${telemetry.jitter.toFixed(0)}ms · perda ${telemetry.lossPercent.toFixed(0)}% · correção máx. ${(maxCorrection*100).toFixed(0)}cm · saltos ${snaps}${state.gameOver?' · FIM DE PARTIDA':me?.health===0?' · VOCÊ MORREU':''}`;
      ...
      for(const enemy of state.enemies)if(!enemies.has(enemy.id)){const avatar=new Avatar(true);avatar.root.position.copy(enemy.position);if(enemy.kind==='boss')avatar.root.scale.setScalar(C.boss.scale);enemies.set(enemy.id,avatar);scene.add(avatar.root);}
  ...
  window.addEventListener('pagehide',()=>{pageLeaving=true;clearTimeout(reconnectTimer);clearInterval(restartTimer);clearInterval(reviveTimer);clearInterval(pingTimer);socket.close();});
  ...
  function frame(now:number){
    ...
    effects.update(dt);if(connected&&snapshot){const me=snapshot.players.find(player=>player.id===id);audio.update(dt,snapshot.phase==='horde',me?Math.hypot(me.velocity.x,me.velocity.z):0,me?.crouch??false,!!snapshot.boss);(scene.background as T.Color).lerp(new T.Color(snapshot.phase==='horde'?0x737b78:0xf3f1e9),dt*1.35);(scene.fog as T.Fog).color.copy(scene.background as T.Color);light.intensity=T.MathUtils.damp(light.intensity,snapshot.phase==='horde'?.85:2.3,1.8,dt);}outline.render(scene,camera.camera);
  }
