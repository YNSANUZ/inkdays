...
export function mountCoopPreview(app:HTMLElement){
  ...
  const avatars=new Map<string,Avatar>(),inputClock=new InputClock(),reliableName=new ReliablePlayerName(),healthEvents=new HealthEvents(),predictedShots=new PredictedShotFeedback();
  ...
  const connect=()=>{
    ...
    if(packet.type==='snapshot'){
      ...
      for(const shot of shotEvents.consume(state.shots)){effects.shot(new T.Vector3().copy(shot.from),new T.Vector3().copy(shot.to));if(shot.hit)effects.impact(new T.Vector3().copy(shot.to));if(shot.player!==id||!predictedShots.confirm(shot.shotId,receivedAt))audio.cue('shot');}
