...
export function createMovementServer(world:CollisionWorld,port=8787,authority:ServerAuthority=new MovementAuthority(world),reconnectGraceMs=5000,conditions?:NetworkConditions,host='127.0.0.1',inactivityMs=7000,runtime?:ServerRuntime){
  ...
  server.on('connection',(socket,request)=>{
    const requestUrl=new URL(request.url??'/',`ws://${request.headers.host??'localhost'}`),resume=requestUrl.searchParams.get('resume');
    ...
    socket.on('message',data=>{
      ...
      link.schedule('inbound',()=>{if(!session!.connected||socket.readyState!==WebSocket.OPEN)return;try{const packet=JSON.parse(data.toString());if(packet?.type==='ping'&&typeof packet.nonce==='number'&&Number.isFinite(packet.nonce)){send(socket,JSON.stringify({type:'pong',nonce:packet.nonce,serverTick:authority.tick}));return;}if(packet?.type==='ready'&&Object.keys(packet).length===1){if(!authority.setReady?.(id,true))send(socket,JSON.stringify({type:'rejected'}));return;}if(packet?.type==='restart'&&Object.keys(packet).length===1){if(!authority.restart?.(id))send(socket,JSON.stringify({type:'rejected'}));return;}if(packet?.type==='revive'&&Object.keys(packet).length===1){if(!authority.revive?.(id))send(socket,JSON.stringify({type:'rejected'}));return;}if(packet?.type==='revive-ally'&&Object.keys(packet).length===2&&typeof packet.target==='string'){if(!authority.reviveAlly?.(id,packet.target))send(socket,JSON.stringify({type:'rejected'}));return;}if(packet?.type==='name'&&Object.keys(packet).length===2){if(!authority.rename?.(id,packet.name))send(socket,JSON.stringify({type:'rejected'}));return;}if(packet?.type==='chat'&&Object.keys(packet).length===3){if(!authority.chat?.(id,packet.messageId,packet.text))send(socket,JSON.stringify({type:'rejected'}));return;}authority.setReady?.(id,true);if(!authority.receive(id,packet))send(socket,JSON.stringify({type:'rejected'}));}
