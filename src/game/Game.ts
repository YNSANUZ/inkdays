...
export class Game {
  ...
  constructor(app:HTMLElement) {
    ...
    this.world=new World(this.scene);this.bossTeaser=new BossTeaser();this.scene.add(this.bossTeaser.root,this.player.avatar.root,this.camera.camera);this.viewWeapon=new FirstPersonWeapon(this.camera.camera);this.enemies=new Enemies(this.scene,this.world);this.effects=new Effects(this.scene);this.ui=new UI(app);this.ui.bindAdminDay(day=>this.start(day));
  ...
  private step(dt:number) {
    ...
    if(event==='horde'){this.horde.begin(this.cycle.day);this.hunters.begin(this.cycle.day);if(this.cycle.day%C.day.bossInterval===0)this.enemies.spawnBoss(this.cycle.day,this.player.position);this.audio.cue('enemy');this.ui.toast(this.cycle.day%C.day.bossInterval===0?`CHEFÃO · ${this.cycle.day%(C.day.bossInterval*2)===0?C.humanDeer.name:C.boss.name} CHEGOU`:'A HORDA CHEGOU · Elimine todos os Borrões.');}
  ...
  private frame=(ms:number)=>{
    ...
    if(this.mode==='playing') {
      ...
      const commons=this.enemies.active.filter(enemy=>enemy.kind==='horde').length,hunters=this.enemies.active.filter(enemy=>enemy.kind==='hunter').length,hordeState=this.horde.state(commons);this.ui.update(dt,this.cycle,this.pistol,this.player.health.value,this.kills,this.money,this.lastCommand.crouch,this.lastCommand.run,{...hordeState,remaining:hordeState.remaining+this.hunters.remaining(hunters),assist:this.hordeAssist});
