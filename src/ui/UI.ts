import { getOfflineAvailability, installGame } from './Install';
...
export class UI {
  ...
  constructor(app:HTMLElement) {
    ...
    window.addEventListener('inkdays-offline-availability',()=>this.updateOfflineAvailability());
  ...
  private updateOfflineAvailability(){const badge=this.root.querySelector<HTMLElement>('#offline-status');if(!badge)return;const state=getOfflineAvailability();badge.textContent=state==='ready'?'OFFLINE · PRONTO':state==='preparing'?'OFFLINE · PREPARANDO':'OFFLINE';badge.dataset.state=state;}
  ...
  settings(s:Settings,change:(s:Settings)=>void,back:()=>void) {
    this.overlay.className='overlay modal';this.overlay.innerHTML=`<div class="card settings-card"><span class="eyebrow">DO SEU JEITO</span><h2>Configurações</h2><label>VOLUME <output id="volume-value">${Math.round(s.volume*100)}%</output><input id="volume" type="range" min="0" max="1" step=".05" value="${s.volume}"></label><label>SENSIBILIDADE <output id="sensitivity-value">${s.sensitivity.toFixed(1)}</output><input id="sensitivity" type="range" min=".2" max="2" step=".1" value="${s.sensitivity}"></label><label>QUALIDADE GRÁFICA<select id="quality"><option value="high" ${s.quality==='high'?'selected':''}>Alta · contornos mais suaves</option><option value="low" ${s.quality==='low'?'selected':''}>Baixa · priorizar desempenho</option></select></label><button class="secondary" id="fullscreen">ALTERNAR TELA CHEIA ↗</button><p id="settings-note">Configurações salvas neste navegador.</p><p class="audio-credit">Música de horda: Dark Ambience Loop · Iwan Gabovitch · qubodup.net</p><button class="primary" id="back">VOLTAR <span>←</span></button></div>`;
