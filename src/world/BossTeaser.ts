...
const shadow=new T.MeshBasicMaterial({color:0x20231f,fog:true});
...
export class BossTeaser{
  ...
  constructor(){
    this.root.name='boss-teaser-silhouette';this.root.userData.silhouetteOnly=true;this.root.position.set(72,0,-28);this.root.rotation.y=-.18;
  ...
  private async install(){
    ...
    content.scale.setScalar(2.15/Math.max(.001,size.y));content.updateMatrixWorld(true);bounds.setFromObject(content);const center=bounds.getCenter(new T.Vector3());content.position.set(-center.x,-bounds.min.y,-center.z);
