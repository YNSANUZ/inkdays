...
describe('predição e reconciliação',()=>{
  ...
  it('repete o identificador do clique até confirmação sem alterar fogo contínuo',()=>{const client=new ClientPrediction(world,{acknowledged:-1,position:{x:0,y:0,z:0},velocity:{x:0,y:0,z:0},vertical:0});const base=command(false);expect(client.prepare(5,{...base,shot:true}).fire).toBe(false);expect(client.shotId).toBe(5);client.prepare(6,base);expect(client.shotId).toBe(5);client.reconcile({acknowledged:5,position:{x:0,y:0,z:0},velocity:{x:0,y:0,z:0},vertical:0});expect(client.shotId).toBeUndefined();expect(client.prepare(7,{...base,fire:true}).fire).toBe(true);});
});
