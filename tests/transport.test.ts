...
it('compensa o alvo visto no transporte sem deixá-lo no passado',async()=>{
  const authority=new CombatAuthority(),instance=createMovementServer(authority.world,0,authority,5000,{latencyMs:8,jitterMs:5,dropEvery:13,duplicateEvery:7,reorderEvery:5},'127.0.0.1',7000,{pulseMs:3,stepsPerPulse:1});const sockets:WebSocket[]=[];
  try{await new Promise<void>(resolve=>instance.server.once('listening',resolve));const address=instance.server.address();if(!address||typeof address==='string')throw Error('Endereço inválido');const end=Date.now()+10000;
    const connect=()=>{const socket=new WebSocket(`ws://127.0.0.1:${address.port}`);sockets.push(socket);const state={id:'',frames:new Map<number,ReturnType<CombatAuthority['snapshot']>>()};socket.on('message',raw=>{const packet=JSON.parse(raw.toString());if(packet.type==='welcome')state.id=packet.id;if(packet.type==='snapshot')state.frames.set(packet.tick,packet);});return {socket,state};},a=connect(),b=connect();while(Date.now()<end&&(!a.state.id||!b.state.id||authority.snapshot().players.length<2))await new Promise(r=>setTimeout(r,10));
    authority.enemies.spawn(1,new Vector3(0,0,10));const enemy=authority.enemies.active[0];enemy.avatar.root.position.set(.85,0,0);enemy.speed=0;while(authority.tick<10)await new Promise(r=>setTimeout(r,5));const viewTick=authority.tick;while(authority.tick===viewTick)await new Promise(r=>setTimeout(r,2));enemy.avatar.root.position.set(10,0,0);a.socket.send(JSON.stringify({version:1,sequence:0,yaw:0,pitch:0,viewTick,command:{x:0,z:0,run:false,crouch:false,jump:false,fire:true,reload:false}}));
...
it('recupera clique de tiro perdido sem duplicar munição',async()=>{
  ...
  try{await new Promise<void>(resolve=>instance.server.once('listening',resolve));const address=instance.server.address();if(!address||typeof address==='string')throw Error('Endereço inválido');const end=Date.now()+10000;
    const connect=()=>{const socket=new WebSocket(`ws://127.0.0.1:${address.port}`);sockets.push(socket);const state={id:'',frames:new Map<number,ReturnType<CombatAuthority['snapshot']>>()};socket.on('message',raw=>{const packet=JSON.parse(raw.toString());if(packet.type==='welcome')state.id=packet.id;if(packet.type==='snapshot')state.frames.set(packet.tick,packet);});return {socket,state};},a=connect(),b=connect();while(Date.now()<end&&(!a.state.id||!b.state.id||authority.snapshot().players.length<2))await new Promise(r=>setTimeout(r,10));const command={x:0,z:0,run:false,crouch:false,jump:false,fire:false,reload:false};
    a.socket.send(JSON.stringify({type:'ping',nonce:1}));a.socket.send(JSON.stringify({type:'ping',nonce:2}));a.socket.send(JSON.stringify({version:1,sequence:0,yaw:0,pitch:0,shotId:7,command}));a.socket.send(JSON.stringify({version:1,sequence:1,yaw:0,pitch:0,shotId:7,command}));while(Date.now()<end&&authority.snapshot().players.find(player=>player.id===a.state.id)?.ammo!==7)await new Promise(r=>setTimeout(r,10));expect(authority.snapshot().shots).toHaveLength(1);
    const firstTick=authority.tick;while(authority.tick<firstTick+20)await new Promise(r=>setTimeout(r,5));a.socket.send(JSON.stringify({version:1,sequence:2,yaw:0,pitch:0,shotId:7,command}));a.socket.send(JSON.stringify({version:1,sequence:3,yaw:0,pitch:0,shotId:7,command}));await new Promise(r=>setTimeout(r,80));expect(authority.snapshot().players.find(player=>player.id===a.state.id)?.ammo).toBe(7);expect(authority.snapshot().shots).toHaveLength(1);
    let common:ReturnType<CombatAuthority['snapshot']>|undefined;while(Date.now()<end&&!common){for(const [tick,left] of a.state.frames){const right=b.state.frames.get(tick);if(right&&left.shots.length===1&&left.players.find(player=>player.id===a.state.id)?.ammo===7){expect(left).toEqual(right);common=left;break;}}if(!common)await new Promise(r=>setTimeout(r,10));}expect(common!.shots[0]).toMatchObject({player:a.state.id,serial:1});
  }finally{for(const socket of sockets)socket.terminate();await instance.close();}
},15000);
