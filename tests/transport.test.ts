...
it('sincroniza derrota coletiva e congela o estado da partida',async()=>{
  ...
  try{await new Promise<void>(resolve=>instance.server.once('listening',resolve));const address=instance.server.address();if(!address||typeof address==='string')throw Error('Endereço inválido');
    ...
    const identities=ended!.players.map(player=>player.id);let restarted:ReturnType<CombatAuthority['snapshot']>|undefined;while(Date.now()<end&&!restarted){sockets[0].send(JSON.stringify({type:'restart'}));await new Promise(r=>setTimeout(r,20));for(const [tick,left] of a){const right=b.get(tick);if(right&&!left.gameOver&&tick>later!.tick){expect(left).toEqual(right);restarted=left;break;}}}expect(restarted).toMatchObject({day:1,phase:'day',gameOver:false,enemies:[],shots:[],players:[{health:100,ammo:8,reserve:48,kills:0,money:0},{health:100,ammo:8,reserve:48,kills:0,money:0}]});expect(restarted!.players.map(player=>player.id)).toEqual(identities);
  }finally{for(const socket of sockets)socket.terminate();await instance.close();}
},20000);
