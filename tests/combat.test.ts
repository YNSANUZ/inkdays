...
describe('combate controlado pelo servidor',()=>{
  ...
  it('alterna para o Cervo Humano no segundo encontro de chefão',()=>{const a=new CombatAuthority();a.join('a');expect(a.enemies.spawnBoss(20,new Vector3(0,0,10))).toBe(true);const boss=a.enemies.active[0];expect(boss).toMatchObject({kind:'boss',bossVariant:'human-deer',speed:C.humanDeer.speed,damage:C.humanDeer.damage});expect(a.snapshot().boss).toMatchObject({name:'O CERVO HUMANO',variant:'human-deer'});});
  it('telegrafa e resolve o rugido do Cervo Humano no servidor',()=>{const a=new CombatAuthority();a.join('a');a.enemies.spawnBoss(20,new Vector3(0,0,10));const boss=a.enemies.active[0];boss.avatar.root.position.set(0,0,10);boss.speed=0;boss.specialCooldown=0;a.step();expect(a.snapshot().boss?.slam).toMatchObject({attack:'roar',radius:C.humanDeer.roarRadius});for(let n=0;n<110;n++)a.step();expect(a.snapshot().players[0].health).toBe(100-C.humanDeer.roarDamage);expect(a.snapshot().impacts.at(-1)).toMatchObject({bossId:boss.id,kind:'roar',radius:C.humanDeer.roarRadius});});
