...
describe('combate controlado pelo servidor',()=>{
  ...
  it('avisa o impacto do Colosso e aplica o dano em área somente pelo servidor',()=>{const a=new CombatAuthority();a.join('a');a.join('b');a.enemies.spawnBoss(10,new Vector3(0,0,10));const boss=a.enemies.active[0];boss.avatar.root.position.set(0,0,10);boss.speed=0;boss.specialCooldown=0;a.step();const warning=a.snapshot().boss?.slam;expect(warning).toMatchObject({serial:1,radius:C.boss.slamRadius});expect(warning!.remaining).toBeGreaterThan(1);expect(a.snapshot().impacts).toHaveLength(0);for(let n=0;n<80;n++)a.step();const state=a.snapshot(),players=state.players;expect(players[0].health).toBe(100-C.boss.slamDamage);expect(players[1].health).toBe(100-C.boss.slamDamage);expect(state.boss?.slam).toBeNull();expect(state.impacts).toHaveLength(1);expect(state.impacts[0]).toMatchObject({serial:1,bossId:boss.id,position:{x:0,y:0,z:10},radius:C.boss.slamRadius});expect(state.impacts[0].tick).toBeGreaterThan(1);});
