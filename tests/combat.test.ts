...
describe('combate controlado pelo servidor',()=>{
  ...
  it('cria um chefão autoritativo no Dia 10 e paga sua recompensa ao autor da eliminação',()=>{
    ...
    expect(state.boss).toMatchObject({id:boss.id,name:'O LAGARTO',variant:'lizard',health:boss.health,maxHealth:boss.maxHealth});
  ...
  it('mantém o Lagarto em todos os encontros de chefão',()=>{const a=new CombatAuthority();a.join('a');for(const day of [10,20,30]){a.enemies.clear();expect(a.enemies.spawnBoss(day,new Vector3(0,0,10))).toBe(true);const boss=a.enemies.active[0];expect(boss).toMatchObject({kind:'boss',bossVariant:'lizard',speed:C.boss.speed,damage:C.boss.damage});expect(a.snapshot().boss).toMatchObject({name:'O LAGARTO',variant:'lizard'});}});
