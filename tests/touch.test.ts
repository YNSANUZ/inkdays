...
import { touchDevice } from '../src/input/TouchControls';
import { joystick,TouchState } from '../src/input/TouchState';
describe('comandos touch',()=>{
  afterEach(()=>vi.unstubAllGlobals());
  it('não força touch em computador com mouse e tela sensível ao toque',()=>{
    vi.stubGlobal('matchMedia',()=>({matches:false}));
    vi.stubGlobal('navigator',{maxTouchPoints:10});
    vi.stubGlobal('location',{search:''});
    expect(touchDevice()).toBe(false);
  });
  it('mantém controles no celular com apontador touch principal',()=>{
  ...
  it('preserva toques rápidos entre dois frames e não repete salto',()=>{const t=new TouchState();t.trigger('fire');t.trigger('jump');expect(t.consume()).toMatchObject({fire:false,shot:true,jump:true});expect(t.consume()).toMatchObject({fire:false,shot:false,jump:false});});
  it('zera todos os comandos ao pausar ou cancelar',()=>{const t=new TouchState();t.x=1;t.fire=true;t.run=true;t.crouch=true;t.trigger('jump');t.trigger('reload');t.clear();expect(t.consume()).toEqual({x:0,z:0,run:false,crouch:false,fire:false,jump:false,reload:false,shot:false});});
});
